...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=15, verbose=False,
            ...
            self.square = SquareAttack(self.model, p_init=.8, n_queries=10, eps=self.epsilon, norm=self.norm,
                n_restarts=1, seed=self.seed, verbose=False, device=self.device, resc_schedule=False)#5000
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            self.attacks_to_run = ['apgd-ce']#'apgd-ce', 'apgd-t', 'fab-t', 'square']
            if self.norm in ['Linf', 'L2']:
                self.apgd.n_restarts = 1
                self.apgd_targeted.n_target_classes = 11##########################################
            ...
            #         version, self.norm))
            self.attacks_to_run = ['square']#'apgd-ce', 'apgd-t', 'fab-t', 'square']
            if self.norm in ['Linf', 'L2']:
                self.apgd.n_restarts = 1
                self.apgd_targeted.n_target_classes = 11##########################################
            elif self.norm in ['L1']:
            ...
            self.fab.n_restarts = 1
            self.apgd_targeted.n_restarts = 1
            self.fab.n_target_classes = 10
            #self.apgd_targeted.n_target_classes = 9
            self.square.n_queries = 3000
