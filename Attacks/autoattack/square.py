...
class SquareAttack():
    ...
    def attack_single_run(self, x, y):
        ...
        def accCaculate(x, y):
            ...
            # num = 0
            # advPredic = self.predict(x)
            # _, predicted = torch.max(advPredic.data, 1)
            # if predicted == y:
            #     num = num + 1
            # return round(float(num / len(y)), 4)
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                x_best = torch.clamp(x + self.eps * self.random_choice(
                    [x.shape[0], h, w]), -1., 1.)  # x.shape[0], c, 1, w])
                # print(x.shape, x_best.shape, "xbest!")
                ...
                for i_iter in range(self.n_queries):
                    ...
                    # print(x_best_curr.shape, new_deltas.shape, '!!!!!!!!!!!!!!!!!!')
                    ...
                    #     model=args.model,
                    #     name=args.name,
                    #     mis=1- round(float((ind_succ.numel()) / n_ex_total), 4))
                    # mis = round(float((ind_succ.numel()) / n_ex_total), 4)

                    robacc_square.append( 1- round(float((ind_succ.numel()) / n_ex_total), 4))
                    # robacc_square.append(accCaculate(ind_succ, y))

                    # if len(res_acc) == 0:
                    #     res_acc = mis
                    # else:

                    # robacc_square.append(mis)
                    ...
                    if ind_succ.numel() == n_ex_total:
                        break
                    np.save("/home/zjut/public/signal/wzw/KD/Attacks/autoattack/square/{}/{}_robacc_square_{}_eps{}.npy"
                            .format(args.model, args.note, self.n_queries, args.epsilon), robacc_square)
...
def write_result_to_csv(**kwargs):
    results = pathlib.Path("../Attacks/autoattack") / "results.csv"
