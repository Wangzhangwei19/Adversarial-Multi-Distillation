...
class DeepFoolAttack(Attack):
    ...
    def perturbation_single(self, sample, device):
        ...
        if args.model == "MCLDNN":
            prediction = torch.unsqueeze(self.model(var_sample),dim=0)
        else:
        ...
        while (original == current) and (iteration < self.max_iterations):
            ...
            self.model.train()
            if args.model == "MCLDNN":
                f_kx = torch.unsqueeze(self.model(var_sample),dim=0)
            else:
    ...
    def perturbation(self, xs, device):
        ...
        for i in range(len(xs)):
            ...
            # adv_labels = predict(model=self.model, samples=adv_samples, device=device)
